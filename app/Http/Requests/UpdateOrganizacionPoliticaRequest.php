...
class UpdateOrganizacionPoliticaRequest extends FormRequest
{
    ...
    {
        $organizacionId = $this->route('organizacion')->id_partido;

        return [
            'codigo_tse' => ['required', 'string', 'max:3', Rule::unique('organizaciones_politicas', 'codigo_tse')->ignore($organizacionId, 'id_partido')],
            'nombre' => 'required|string|max:100',
            'sigla' => ['required', 'string', 'max:10', Rule::unique('organizaciones_politicas', 'sigla')->ignore($organizacionId, 'id_partido')],
